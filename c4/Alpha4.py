...
class Alpha4():
    ...
    def init_A4(self, c4_shape,n_actions):
        ...
        conv_1 = tf.keras.layers.Conv2D(16, (4, 4), strides=(1, 1), \
                                        activation=tf.nn.relu)(board_input)
        ...
        hidden = tf.keras.layers.Dense(64, activation=tf.nn.relu)\
                                      (conv_flattened)
        # 2 outputs : policy (ie distribution over moves) and vf (-1:1)
        output_policy = tf.keras.layers.Dense(
                        n_actions, activation=tf.nn.softmax, name='policy')(hidden)
        output_value = tf.keras.layers.Dense(\
            1, activation=tf.nn.tanh, name='value_fun')(hidden)
